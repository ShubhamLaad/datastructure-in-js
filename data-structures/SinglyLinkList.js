...
class SinglyLinkedList {
  ...
  get(index) {
    ...
    for (
      var current = this.head, currentIndex = 0;
  ...
  rotate(rotateCount) {
    ...
    if (!this.head || !rotateCount || Math.abs(rotateCount) > this.length)
      return this;
    let current = this.head;
    if (rotateCount < 0) {
      // rotateCount decide current so make it positive
      rotateCount = this.length + rotateCount;
    }
    for (
      let i = 1;
      current && i <= rotateCount - 1;
